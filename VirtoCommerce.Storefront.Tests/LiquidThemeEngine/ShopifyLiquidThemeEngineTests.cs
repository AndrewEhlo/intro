...
#pragma warning restore 618
        ...
        {
            var postRenderTemplateChangeConfigObject = new Dictionary<string, string>()
            {
                { "PostRenderTemplateChange:ExternalLinks:UrlWhitelist[0]", "https://test.com" }
            };

            var postRenderTemplateChangeConfig = new ConfigurationBuilder()
               .AddInMemoryCollection(postRenderTemplateChangeConfigObject)
               .Build();

            return new ShopifyLiquidThemeEngine(MemoryCache, GetWorkContextAccessor(useThemesInheritance), HttpContextAccessor,
                null, ContentBlobProvider, null, new OptionsWrapper<LiquidThemeEngineOptions>(options), new FeaturesAgent(), new PostRenderTemplateChanger(GetWorkContextAccessor(false), postRenderTemplateChangeConfig));
        }
